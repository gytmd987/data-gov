...
class Settings(BaseSettings):
    ...
    # ⚠️ 추론 모델은 **추론 토큰도 이 한도를 함께 쓴다.** 한도가 빠듯하면 생각만 하다
    # 끝나 본문이 비어서 온다(finish_reason="length"). 그래서 넉넉히 잡는다 — 상한일
    # 뿐이라 모델이 일찍 멈추면 남는 만큼은 비용이 없다.
    vllm_max_tokens: int = 1500       # 채팅 답변 상한
    vllm_task_max_tokens: int = 2000  # 도구 선택·SQL 생성 등 구조화 응답 상한
    ...
    # 리랭커를 아예 끄는 스위치. 리랭커가 CPU 로 돌면(GPU 이미지 문제 등) 질문 하나에
    # 8초 넘게 걸린다 — 크로스 인코더는 후보마다 모델을 한 번씩 돌리기 때문이다.
    # 끄면 하이브리드 검색(dense+BM25) 융합 순위를 그대로 쓴다. 정확도는 조금 떨어지지만
    # 8초가 0초가 된다. GPU 를 고칠 때까지의 임시 방편으로 쓴다.
